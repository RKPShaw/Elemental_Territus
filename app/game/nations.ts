import { ELEMENTS, ELEMENT_ORDER } from "./elements";
import type { ElementDefinition, ElementId, NationId } from "./types";

/**
 * The roster of competing nations.
 *
 * A nation is not the same thing as an element. Ten nations share each element,
 * so they share its matchups, favoured terrain and temperament while competing
 * as separate powers with their own territory, treasury and diplomacy. Element
 * is what a nation *is*; nation is who it *is*.
 *
 * The roster is fixed at module load, exactly as ELEMENT_ORDER was, so
 * iteration order is stable and the simulation stays deterministic.
 */
export const NATIONS_PER_ELEMENT = 10;

/** Roman numerals read better than digits on a map label. */
const ORDINALS = ["I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X"] as const;

function buildRoster(): NationId[] {
  const roster: NationId[] = [];
  // Grouped by element rather than interleaved, so a listing reads as five
  // families. Draft order is decided separately and deliberately.
  for (const element of ELEMENT_ORDER) {
    for (let ordinal = 0; ordinal < NATIONS_PER_ELEMENT; ordinal += 1) {
      roster.push(`${element}-${ordinal + 1}`);
    }
  }
  return roster;
}

export const NATION_ORDER: readonly NationId[] = buildRoster();

export const NATION_COUNT = NATION_ORDER.length;

function hexToRgb(hex: string): [number, number, number] {
  const value = Number.parseInt(hex.slice(1), 16);
  return [(value >> 16) & 255, (value >> 8) & 255, value & 255];
}

function rgbToHex(red: number, green: number, blue: number): string {
  const channel = (value: number) => Math.max(0, Math.min(255, Math.round(value)))
    .toString(16)
    .padStart(2, "0");
  return `#${channel(red)}${channel(green)}${channel(blue)}`;
}

/**
 * Ten distinguishable tints per element, walking from the element's soft colour
 * to its deep one. A nation therefore reads as its family at a glance while
 * still being separable from its siblings.
 */
function shadeFor(element: ElementId, ordinal: number): string {
  const [softRed, softGreen, softBlue] = hexToRgb(ELEMENTS[element].softColor);
  const [deepRed, deepGreen, deepBlue] = hexToRgb(ELEMENTS[element].deepColor);
  const amount = NATIONS_PER_ELEMENT === 1 ? 0.5 : ordinal / (NATIONS_PER_ELEMENT - 1);
  return rgbToHex(
    softRed + (deepRed - softRed) * amount,
    softGreen + (deepGreen - softGreen) * amount,
    softBlue + (deepBlue - softBlue) * amount,
  );
}

interface NationDefinition {
  id: NationId;
  element: ElementId;
  /** Position within its element's family, from zero. */
  ordinal: number;
  /** Short label for tables and legends, such as "Ember IV". */
  name: string;
  /** Full title, such as "The Cinderkin IV". */
  realmName: string;
  color: string;
}

function buildDefinitions(): Record<NationId, NationDefinition> {
  const definitions: Record<NationId, NationDefinition> = {};
  for (const element of ELEMENT_ORDER) {
    for (let ordinal = 0; ordinal < NATIONS_PER_ELEMENT; ordinal += 1) {
      const id = `${element}-${ordinal + 1}`;
      definitions[id] = {
        id,
        element,
        ordinal,
        name: `${ELEMENTS[element].name} ${ORDINALS[ordinal] ?? ordinal + 1}`,
        realmName: `${ELEMENTS[element].realmName} ${ORDINALS[ordinal] ?? ordinal + 1}`,
        color: shadeFor(element, ordinal),
      };
    }
  }
  return definitions;
}

export const NATIONS: Record<NationId, NationDefinition> = buildDefinitions();

export function nationElement(id: NationId): ElementId {
  return NATIONS[id]!.element;
}

/** The elemental character behind a nation: matchups, terrain, temperament. */
export function nationElementDefinition(id: NationId): ElementDefinition {
  return ELEMENTS[nationElement(id)];
}

export function nationsOfElement(element: ElementId): NationId[] {
  return NATION_ORDER.filter((id) => nationElement(id) === element);
}

/**
 * Draft order for choosing start locations: a snake, reversing every round.
 *
 * Picking strictly in roster order would hand the whole first element the ten
 * best sites. Rotating the element each pick spreads the families, and
 * reversing each round means the nation that picked last gets the first pick of
 * the next round, so no seat is systematically disadvantaged.
 */
export function draftOrder(): NationId[] {
  const order: NationId[] = [];
  for (let round = 0; round < NATIONS_PER_ELEMENT; round += 1) {
    const elements = round % 2 === 0 ? ELEMENT_ORDER : [...ELEMENT_ORDER].reverse();
    for (const element of elements) {
      order.push(`${element}-${round + 1}`);
    }
  }
  return order;
}
